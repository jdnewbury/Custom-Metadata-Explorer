...
def process_custom_fields():
    ...
    # Create Header
    header = ['Object', 'APIName', 'UniqueKey', 'Label', 'Description', 'FieldType', 'ExternalId', 'Required', 'FeedHistoryTracking', 'HistoryTracking', 'ReferenceTo', 'RelationshipName']
    ws.append(header)
    ...
    for root_dir, dirs, files in os.walk(processing_path):
        for file in files:
            if not file.endswith('__mdt') and not file.endswith('__e'):
                ...
                if '__c.field-meta' in file and 'HealthCloudGA' not in file and 'Knowledge__kav' not in file_path:
