...
def process_permission_sets():
    metadata_folder = 'permissionsets'
    ...
    for root_dir, dirs, files in os.walk(processing_path):
        ...
        for file in files:
            ...
            hasActivationRequiredVar = root.find('.//{http://soap.sforce.com/2006/04/metadata}hasActivationRequired').text if root.find('.//{http://soap.sforce.com/2006/04/metadata}hasActivationRequired') is not None else ""
            ...
            licenseVar = root.find('.//{http://soap.sforce.com/2006/04/metadata}license').text if root.find('.//{http://soap.sforce.com/2006/04/metadata}license') is not None else ""

            ws.cell(row=i,column=1).value = labelVar
            ws.cell(row=i,column=2).value = hasActivationRequiredVar
            ws.cell(row=i,column=3).value = descriptionVar
            ws.cell(row=i,column=4).value = licenseVar

            userPermissions = root.findall('.//{http://soap.sforce.com/2006/04/metadata}userPermissions')
            j = 0
            userPermissions_text = ""
            for item in userPermissions:
                j += 1
                if j > 1:
                    userPermissions_text += '\n'
                if (enabled := item.find('.//{http://soap.sforce.com/2006/04/metadata}enabled')) is not None:
                    userPermissions_text += f'enabled: {enabled.text}, '
                if (name := item.find('.//{http://soap.sforce.com/2006/04/metadata}name')) is not None:
                    userPermissions_text += f'name: {name.text} '

            # Step 6
            ws.cell(row=i,column=5).value = userPermissions_text
            ...
            # Step 6
            ws.cell(row=i,column=6).value = objectPermissions_text
    ...
    if ws.max_row > 1:
        ...
        for column_cells in ws.columns:
            ...
            ws.column_dimensions['F'].width = 140
            for cell in ws['F']:
                cell.alignment = Alignment(wrap_text=True)
